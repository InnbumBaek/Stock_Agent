...
function sRoadmap() {
  ...
  const outs = [["코드로 끝난 것", "스키마 · 러너 · 관문 · 트리거 · 사이클 · 되짚기 — 자체 검사 190개", GRN],
...
function sNext() {
  ...
  const todo = [
    ...
    ["ahxz2006 을 어떻게 할지 정한다", "이벤트 러너를 만들어 ritter1991 · fh2001 을 넣었다. 남은 하나는 일간 잔차 변동성이라 러너를 더 만들지, 미검증으로 둘지는 판단이다.", "판단", PUR],
  ...
  const kpi = [["190개", "자체 검사", "키·네트워크·원장 없이 돈다", GRN],
               ["0줄", "ki_monitor.py 변경", "측정층은 손대지 않았다", CYN],
               ["7 / 12", "검정 대상 논문", "대상 아님 4 · 러너 미비 1", AMB],
...
function sExec() {
  ...
  const st = [["주 1회", "논문 사이클", "신규 + 재검 도래분"], ["N", "살아있는 논문", "고정값이 아니다"],
              ["7", "발행 게이트", "재현 관문 신설"], ["0", "원장 쓰기 권한", "에이전트는 못 쓴다"],
              ["0줄", "ki_monitor.py 변경", "측정층은 그대로"], ["190개", "자체 검사", "키·망·원장 없이"]];
  st.forEach((t, i) => {
...
function sStack() {
  ...
  txt(s, [
    { text: "실제로 들어간 것 — agents/ 파이썬 11개 · 데스크 정의 9 · 스킬 5 · 자체 검사 190개. ", options: { bold: true, color: GRN } },
...
function sAppendix() {
  ...
  foot(s, "// 실제로 들어갔다 — agents/ 파이썬 11 · 데스크 정의 9 · 스킬 5 · 자체 검사 190개 · ki_monitor.py 변경 0줄");
}
