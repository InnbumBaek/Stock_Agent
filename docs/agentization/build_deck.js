...
function sRoadmap() {
  ...
  const outs = [["코드로 끝난 것", "스키마 · 러너 · 관문 · 트리거 · 사이클 · 되짚기 — 자체 검사 212개", GRN],
...
function sNext() {
  ...
  const kpi = [["212개", "자체 검사", "키·네트워크·원장 없이 돈다", GRN],
...
function sExec() {
  ...
  const st = [["주 1회", "논문 사이클", "신규 + 재검 도래분"], ["N", "살아있는 논문", "고정값이 아니다"],
              ["7", "발행 게이트", "재현 관문 신설"], ["0", "원장 쓰기 권한", "에이전트는 못 쓴다"],
              ["0줄", "ki_monitor.py 변경", "측정층은 그대로"], ["212개", "자체 검사", "키·망·원장 없이"]];
  st.forEach((t, i) => {
...
function sStack() {
  ...
  txt(s, [
    { text: "실제로 들어간 것 — agents/ 파이썬 11개 · 데스크 정의 9 · 스킬 5 · 자체 검사 212개. ", options: { bold: true, color: GRN } },
...
function sAppendix() {
  ...
  mono(s, [
    ...
    '│',
    '├─ agents/               ← 들어감 (자체 검사 212개)',
    '│   ├─ envelope.py          봉투 스키마 · 검증기      25',
  ...
  foot(s, "// 실제로 들어갔다 — agents/ 파이썬 11 · 데스크 정의 9 · 스킬 5 · 자체 검사 212개 · ki_monitor.py 변경 0줄");
}
